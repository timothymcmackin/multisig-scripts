...
BAKER="tz1X2jPbN1V6Yae7gf1NcbR4DgMKrouHrrrA" # zir0h

pop=$(cat "$pop_filename")
multi_pk=$(cat "$pk_filename")

# network_cmd="octez-client -E https://rpc.shadownet.teztnets.com config update"
# echo "$network_cmd"
# eval $network_cmd
...
operation_json="{ \"branch\": $branch,
  \"contents\":
    [ { \"kind\": \"reveal\",
        \"source\": \"$multi_address\", \"fee\": \"735\",
        \"counter\": \"$counter1\", \"gas_limit\": \"3251\", \"storage_limit\": \"0\",
        \"public_key\": \"$multi_pk\",
        \"proof\": \"$pop\" },
      { \"kind\": \"delegation\",
        \"source\": \"$multi_address\", \"fee\": \"160\",
        \"counter\": \"$counter2\", \"gas_limit\": \"100\", \"storage_limit\": \"0\",
        \"delegate\": \"$BAKER\" } ] }"
...
one_sig_full=$(octez-client sign bytes "$operation_bytes_0x03" for "$NN_ONE")
one_sig=$(echo "$one_sig_full" | grep '^Signature:' | awk '{print $2}')
two_sig_full=$(octez-client sign bytes "$operation_bytes_0x03" for "$NN_TWO")
two_sig=$(echo "$two_sig_full" | grep '^Signature:' | awk '{print $2}')
three_sig_full=$(octez-client sign bytes "$operation_bytes_0x03" for "$NN_THREE")
three_sig=$(echo "$three_sig_full" | grep '^Signature:' | awk '{print $2}')

aggregated_json="{
\"public_key\": \"$multi_pk\",
\"message\": \"$operation_bytes_03\",
\"signature_shares\": [ \"$one_sig\", \"$two_sig\", \"$three_sig\" ] }"
